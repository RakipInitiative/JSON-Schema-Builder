(function () {
        ...
        <div class="col-xs-6" style="border-right:1px solid #9E9E9E">\
            ...
            <form class="form-horizontal form-compact model-detail-form" name="detailForm" role="form">\
                ...
                <div class="form-group">\
                    ...
                    <div class="col-xs-9">\
                        <input type="text" class="form-control sm detail-ip" placeholder="Title Name" ng-model="entity._title" ng-change="setDirty()" />\
                    </div>\
                </div>\
                <div class="form-group">\
                <label class="control-label col-xs-2">Ext. Enum:</label>\
                <div class="col-xs-9">\
                    <input type="text" class="form-control sm detail-ip" placeholder="External enum" ng-model="entity._extEnum" ng-change="setDirty()" />\
                </div>\
            </div>\
            </form>\
            </div>\
    ...
    <div ng-show="ctrl.expanded" class="objProps">\
        ...
        <!-- Iterates through the objects properties to list them out -->\
            ...
            <div ng-show="configs.showMoreOptn == \'$ref\' || configs.extraArrayOptn">\
                ...
                <div class="array-items">\
                    <select ng-click="$event.stopPropagation()" ng-change="setModelFor$Ref()" style="height:24px" ng-model="modelRef.model" ng-change="setDirty()" >\
                        <option ng-repeat="(name, m) in $definitions track by $index" value="{{name}}">#/$defs/{{name}}</option>\
                    </select>\
                </div>\
            </div>\
        </div>';
    ...
    function jsonSchemaDirective(JsonSchema, $timeout) {
        return{
            ...
            link: function ($scope, ele, attr, model) {
                initRootElement();
                //On click update defitions
                angular.element('body').bind('click', function (e) {
                    $scope.updateDefinitions();
                });
                ...
                // Called when the array type is set
                ...
                // Called when a def is selected from the dropdown
                ...
                $scope.updateDefinitions = function() {
                    var currentDefName = $scope.$data.defName;
                    if (currentDefName != undefined) {
                        var schema = JsonSchema.obj2schema($scope.$data, $scope.$definitions);
                        $scope.$definitions[currentDefName] = schema;
                    }
                }
                ...
                $scope.convertSchema2Obj = function () {
                    if($scope.$schema && $scope.$schema.original !== $scope.$schema.dup){
                        try{
                            var newSchema = JSON.parse($scope.$schema.original);
                            // Load defs from schema into editor
                            if (newSchema.$defs) {
                                $scope.$definitions = newSchema.$defs;
                                $scope.$childs = {};
                                for (currentDef in $scope.$definitions) {
                                    $scope.$childs[currentDef] = JsonSchema.schema2obj($scope.$definitions[currentDef]);
                                    $scope.$childs[currentDef].root$$ = true;
                                    $scope.$childs[currentDef].defName = currentDef;
                                }
                            }
                            if(newSchema.definitions){
                                $scope.$definitions = angular.merge($scope.$definitions, newSchema.definitions);
                            }
                            $scope.$data = JsonSchema.schema2obj(newSchema, undefined, undefined, true, $scope.$definitions);
                        }catch(e){
    ...
    function JsonSchemaService() {
        ...
        function obj2schema(entity, models) {
            ...
            for(var x = 0; x< entity._type.length; x++){
                ...
                switch (type) {
                    ...
                    case '$ref':
                        ...
                        schema.$ref = (entity._path||'#/$defs/') + (path|| entity._value);
                        delete schema.type;
                }
            }
            ...
            // Common fields
            if (entity._title) {
                schema.title = entity._title;
            }

            if (entity._extEnum) {
                schema.externalEnum = entity._extEnum;
            }
        ...
        function schema2obj(schema, key, required, isRoot, modelObjs, parent) {
            ...
            for(var x=0; x< schema.type.length; x++){
                ...
                if (obj) {
                    if (schema.title) {
                        obj._title = schema.title
                    }

                    if (schema.externalEnum) {
                        obj._extEnum = schema.externalEnum
                    }
                }
